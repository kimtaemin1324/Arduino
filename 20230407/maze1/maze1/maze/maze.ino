...
void loop() 
{
 ...
 delay(500);                // 일정한 시간 동안 정지
   Serial.println("Rotate CCW");
    motor_A_control(LOW,70);  // 오른쪽 후진
    motor_B_control(HIGH,100);  // 왼쪽은 전진
   delay(1240);                 //일정한 시간 동안 회전
  Serial.println("Rotate stop");
  motor_A_control(LOW,0);  // 오른쪽 정지
  motor_B_control(HIGH,0);   // 왼쪽은 정지
 delay(500);               // 일정한 시간 동안 정지
 ...
 90도 회전 반시계 방향으로 회전
   ...
   motor_B_control(HIGH,70);  // 왼쪽은 전진
   delay(700);                 //일정한 시간 동안 회전
   Serial.println("Rotate stop");
   motor_A_control(LOW,0);  // 오른쪽 정지
   motor_B_control(HIGH,0);   // 왼쪽은 정지
   delay(500);               // 일정한 시간 동안 정지
   ...
   // 90도 회전 반시계 방향으로 회전
   Serial.println("Rotate stop");
   motor_B_control(LOW,0);  // 오른쪽 정지
   motor_A_control(HIGH,0);   // 왼쪽은 정지
   delay(500);                // 일정한 시간 동안 정지
   Serial.println("Rotate CCW");
   motor_B_control(LOW,100);  // 오른쪽 후진
   motor_A_control(HIGH,70);  // 왼쪽은 전진
   delay(700);                 //일정한 시간 동안 회전
   Serial.println("Rotate stop");
   motor_A_control(LOW,0);  // 오른쪽 정지
   motor_B_control(HIGH,0);   // 왼쪽은 정지
